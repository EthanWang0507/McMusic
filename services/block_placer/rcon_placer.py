...
class RconBlockPlacer(BlockPlacer):
    def __init__(self, host: str, port: int, pwd: str, auto_unload: bool = True) -> None:
        self.host = host
        self.port = port
        self.pwd = pwd
    ...
    def _load_chunks(self, min_pos: tuple[int, int], max_pos: tuple[int, int]):
        ...
        while cur_cz <= max_cz:
            ...
            self._mcr.command(f"/forceload add {min_x} {load_z_min} {max_x} {load_z_max}")
    ...
    def _unload_chunks(self):
        ...
        for x1, z1, x2, z2 in self._loaded_chunks:
            self._mcr.command(f"/forceload remove {x1} {z1} {x2} {z2}")
        LOGGER.info("区块卸载完成。")
